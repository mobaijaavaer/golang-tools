...
type Function[E comparable, R comparable] func(E) R

// BiFunction 是一个泛型函数类型，用于接受两个参数并返回另一种类型的数据。
type BiFunction[E comparable, W comparable, R comparable] func(E, W) R

// Predicate 是一个泛型函数类型，用于对给定的数据进行判断，返回一个布尔值。
type Predicate[T comparable] func(T) bool

// BiConsumer 是一个泛型函数类型，用于接受两个参数并执行某些操作。
type BiConsumer[E comparable, W comparable] func(E, W)
